...
const CLYDE_SERVER_ID = "clyde-on-desk";
const CLYDE_SERVER_HEADER = "x-clyde-server";
...
function writeRuntimeConfig(port) {
  ...
  const body = JSON.stringify({ app: CLYDE_SERVER_ID, port: safePort }, null, 2);
...
function isClydeResponse(res, body) {
  if (readHeader(res, CLYDE_SERVER_HEADER) === CLYDE_SERVER_ID) return true;
  if (!body) return false;
  try {
    const data = JSON.parse(body);
    return data && data.app === CLYDE_SERVER_ID;
  } catch {
    return false;
  }
}
...
function probePort(port, timeoutMs, callback, options = {}) {
  ...
  const req = httpGet(
    ...
    (res) => {
      ...
      res.on("end", () => callback(isClydeResponse(res, body)));
    }
  );
...
function postStateToPort(port, payload, timeoutMs, callback, options = {}) {
  ...
  const req = httpRequest(
    ...
    (res) => {
      if (readHeader(res, CLYDE_SERVER_HEADER) === CLYDE_SERVER_ID) {
        res.resume();
        callback(true, port);
        return;
      }
      ...
      res.on("end", () => callback(isClydeResponse(res, responseBody), port));
    }
  );
...
function discoverClydePort(options, callback) {
